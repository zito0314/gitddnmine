            ...
            return {
              ...
              children: (
                <Space direction="vertical" size={2}>
                  <Space wrap><Text strong>{activity.actor}</Text><StatusTag status={activity.type?.startsWith('security') ? 'warning' : 'passed'} label={activity.type} /></Space>
                  <Text>{activity.message}</Text>
                  <Text type="secondary">{activity.createdAt} · target {link ? <Link to={link}>{activity.targetId}</Link> : activity.targetId}</Text>
                </Space>
              ),
            }
          })}
