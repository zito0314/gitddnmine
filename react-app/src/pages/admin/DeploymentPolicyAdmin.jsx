import { Card, Flex, Space, Tag } from 'antd'
import { getDeploymentPolicies } from '../../api/admin'
import { PageHeader } from '../../components/common'
import { UI_TEXT } from '../../constants'
import { DemoPolicyForm, PolicyTable } from './AdminShared'

export default function DeploymentPolicyAdmin() {
  const policies = getDeploymentPolicies()

  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <PageHeader
        title={UI_TEXT.adminNavigation.deploymentPolicy}
        description="운영이관 승인자, 배포 가능 시간대, 금지 시간대, 환경별 승인 조건을 관리해요."
      />
      <Card title="Environment gates">
        <Flex gap={8} wrap="wrap">
          <Tag color="blue">dev</Tag>
          <Tag color="gold">staging</Tag>
          <Tag color="red">production</Tag>
        </Flex>
      </Card>
      <DemoPolicyForm title="Deployment governance 규칙" policies={policies} />
      <PolicyTable data={policies} />
    </Space>
  )
}
